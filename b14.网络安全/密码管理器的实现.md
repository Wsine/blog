...
![](https://wsine.cn-gd.ufileos.com/image/wsine-blog-image284.png)
...
![](https://wsine.cn-gd.ufileos.com/image/wsine-blog-image285.png)
...
![](https://wsine.cn-gd.ufileos.com/image/wsine-blog-image286.png)

这里列出的该软件全部的使用方法和版办号，目前的版本号比较低，推出之后版本号会变成1.0正式版

**新用户登录**

![](https://wsine.cn-gd.ufileos.com/image/wsine-blog-image287.png)

新用户登录，新建一个数据文件用户存储，新用户与否取决于是否有用户数据。用户数据的首行存储的是用户名和经过AES加密后的16进制用户密码

**尝试错误密码：**

![](https://wsine.cn-gd.ufileos.com/image/wsine-blog-image288.png)

说明了100分才是该用户的正确登陆密码，0分是不行的。

**添加域名和密码：**

![](https://wsine.cn-gd.ufileos.com/image/wsine-blog-image289.png)

添加键值对，存储的格式是域名，哈希值的16进制，网站密码经过AES加密的16进制形式

**尝试重复添加相同的域名：**

![](https://wsine.cn-gd.ufileos.com/image/wsine-blog-image290.png)

因为已经存储过了，所以会提示已存在

**删除域名密码对：**

![](https://wsine.cn-gd.ufileos.com/image/wsine-blog-image291.png)

查看原本就有的用户数据，然后删除其中一条，然后再重新查看，发现删除成功。

**尝试删除不存在的域名：**

![](https://wsine.cn-gd.ufileos.com/image/wsine-blog-image292.png)

对于不存在的域名，无法删除，提示不存在

**查询指定域名的密码：**

![](https://wsine.cn-gd.ufileos.com/image/wsine-blog-image293.png)

成功查询到记录的密码

**查询不存在的域名：**

![](https://wsine.cn-gd.ufileos.com/image/wsine-blog-image294.png)

查询失败。好想买一个.com域名

**修改指定域名的密码：**

![](https://wsine.cn-gd.ufileos.com/image/wsine-blog-image295.png)

对比前后两次可以发现，指定域名的存储密码已经改变

**修改不存在的域名：**

![](https://wsine.cn-gd.ufileos.com/image/wsine-blog-image296.png)

会提示修改失败，域名不存在

**人为修改用户数据：**

![](https://wsine.cn-gd.ufileos.com/image/wsine-blog-image297.png)
